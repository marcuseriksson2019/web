...
export class Cart {
    ...
    constructor(private router: Router, private cartSource: CartModel, private httpReq: HttpRequests, private accounts: Accounts, private orderstack: Orderstack) { 
        this.VAT = 0;
        this.index = 0;
        this.quantity = 1;
    ...
    placeOrder(userDetails: Order) {
        ...
        if (this.accounts.user) {
            ...
            this.httpReq.postOrder(this.order).subscribe(data => {
                this.order = data;
            });
